# GCP Landing Zone - Sunbird RC

![sunbird-rc-hld](./assets/sunbird-rc-hld.png)

## Introduction

Sunbird RC is a Global DPG that enables the creation of electronic registries, which are secure databases for storing and managing data about individuals or entities. It supports issuing and verifying verifiable credentials, which are secure digital certificates for attestation purposes.

## Features

- **Electronic Registries**: Securely store and manage data about individuals or entities.
- **Attestation Capabilities**: Enable authorized data repositories to publish data in a digitally signed form.
- **Verifiable Credentialing**: Issue and verify secure digital certificates for various applications.

## Objective

The objective of this project is to simplify the deployment process for Sunbird RC. We will achieve this by developing a **one-click installer** that automates deployment across development, staging, and production environments. The project aims to facilitate one-click deployment specifically for **Global DPG's (Digital Public Goods) on Google Cloud Platform**.

In alignment with global trends:

- DPG's  are gaining recognition for their role in advancing technology accessibility and infrastructure.
- According to a recent report by the Economic Times, India is emerging as a key advocate and user of DPGs, highlighting their impact on digital governance and public services ([Economic Times Article](https://economictimes.indiatimes.com/tech/technology/india-a-hotbed-for-startups-and-key-market-for-google-cloud-global-sales-head/articleshow/110944104.cms?from=mdr)).
...
- Integrating deployment functionalities tailored for **Global DPG's (Digital Public Goods) on Google Cloud Platform (GCP)**.
- Providing comprehensive documentation and support resources to assist users in deploying and managing Sunbird RC instances efficiently.
...
- **setup -** Script for setting up of Core Infrastructure
  ...
  - **setup.tf -** deployment of GKE cluster
      - Deploy the Core infrastructure
        ...
        - **Worker pool** - Created specifcially to host Sunbird RC microservices
...
- **post-config -** Script to perform post configurations
  ...
  - **setup.tf -** deployment of GKE cluster
      ...
      - K8s **Namespaces**
        ...
        - **registry** - for hosting all sunbird rc microservices
...
```bash
k create secret generic identity-secrets \
...
```bash
k create secret generic credential-schema-secrets \
--from-literal=DATABASE_URL="" \
-n registry
...
```bash
k create secret generic credential-secrets \
--from-literal=DATABASE_URL="" \
-n registry
...
```bash
k create secret generic keycloak-secrets --from-literal=DB_VENDOR=postgres \
                --from-literal=DB_ADDR= \
...
```bash
k create secret generic registry-secrets \
  --from-literal=sunbird_sso_admin_client_id="" \
  --from-literal=sunbird_sso_client_id="registry-frontend" \
  --from-literal=sunbird_sso_admin_client_secret="" \
  --from-literal=connectionInfo_uri="" \
  --from-literal=connectionInfo_username= \
  --from-literal=connectionInfo_password= \
  --from-literal=elastic_search_username= \
  --from-literal=elastic_search_password= \
  --from-literal=sunbird_keycloak_user_password= \
  -n registry
#k delete secrets/registry-secrets -n registry
```
...
_REGION_="$REGION",_CLUSTER_NAME_="$CLUSTER",\
_LOG_BUCKET_=$PROJECT_ID-tfs-stg,_K8S_NAMESPACE_="registry",\
_HELM_CHART_NAME_="registry-chart-registry",_HELM_CHART_PATH_="deployments/registry/registry-chart/",\
_HELM_VALUES_PATH_="deployments/values-$ENV/registry/values-registry.yaml"

#gcloud builds submit --config="./builds/apps/destroy-script.yaml" \
--project=$PROJECT_ID --substitutions=_PROJECT_ID_=$PROJECT_ID,\
_REGION_="$REGION",_CLUSTER_NAME_="$CLUSTER",_K8S_NAMESPACE_="registry",\
_LOG_BUCKET_=$PROJECT_ID-tfs-stg,_HELM_CHART_NAME_="registry-chart-registry"
```

##### Manual

```bash
helm upgrade --install --create-namespace registry-chart-registry $DEPLOYMENTS_PATH/registry/registry-chart/ -n registry -f $DEPLOYMENTS_PATH/values-$ENV/registry/values-registry.yaml
#helm uninstall registry-chart-registry -n registry
```

#### Registry Ingress
```bash
helm upgrade --install --create-namespace registry-ingress-chart $DEPLOYMENTS_PATH/registry/registry-ingress-chart/ -n registry -f $DEPLOYMENTS_PATH/values-$ENV/registry/values-ingress-registry.yaml
#helm uninstall registry-ingress-chart -n registry
```
#### In Similar way deploy the remianing services.
